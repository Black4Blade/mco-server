 ...
 * @param {Buffer} data
 */
function npsLogoutGameUser (socket) {
 ...
 * @param {Buffer} rawData
 */
function personaDataHandler (rawPacket) {
  const {connection, data} = rawPacket;
  const {sock} = connection;
  const requestCode = data.readUInt16BE (0).toString (16);

  if (requestCode === '503') {
    npsSelectGamePersona (sock);
    return connection;
  }

  if (requestCode === '50f') {
    npsLogoutGameUser (sock);
    return connection;
  }

  if (requestCode === '532') {
    npsGetPersonaMaps (sock, data);
    return connection;
  }
  logger.error (
    `PERSONA: Unknown code ${requestCode} was received on port 8228`
  );
